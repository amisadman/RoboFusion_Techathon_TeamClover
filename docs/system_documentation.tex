...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Smart Campus Safety \& Risk Grid}
\fancyhead[R]{System Documentation}
\fancyfoot[C]{\thepage}
...
All API responses follow a unified JSON envelope format. Below are complete, unbroken payload examples for all 14 backend REST routes.

\needspace{14\baselineskip}
\subsection{1. Microcontroller Telemetry Ingestion (\texttt{POST /api/readings})}
\paragraph{Auth Header}: \texttt{X-Zone-Key: key\_server\_room\_456}
...
\paragraph{Auth}: Active Session Cookie (Staff/Admin)
...
\needspace{14\baselineskip}
\subsection{3. Create New Zone (\texttt{POST /api/zones})}
\paragraph{Auth}: Active Session Cookie (Admin Only)
...
{
  "zone_id": "robotics_lab",
  "name": "Robotics & Automation Lab",
  "hazard_profile": "High voltage motor drivers & lithium battery storage"
}
\end{lstlisting}

\paragraph{Response (201 Created)}:
\begin{lstlisting}[language=json, samepage=true]
{
  "success": true,
  ...
  "data": {
    ...
    "name": "Robotics & Automation Lab",
    "hazard_profile": "High voltage motor drivers & lithium battery storage",
    "api_key": "key_robotics_lab_8921a"
  }
}
\end{lstlisting}

\needspace{14\baselineskip}
\subsection{4. Get Zone API Key (\texttt{GET /api/zones/:id/key})}
\paragraph{Auth}: Active Session Cookie (Admin Only)
...
\begin{lstlisting}[language=json, samepage=true]
{
  ...
  "data": {
    "zone_id": "server_room",
    "api_key": "key_server_room_456"
  }
}
\end{lstlisting}

\needspace{14\baselineskip}
\subsection{5. Manual Zone State Override (\texttt{POST /api/zones/:id/override})}
\paragraph{Auth}: Active Session Cookie (Admin Only)

\paragraph{Request Payload}:
\begin{lstlisting}[language=json, samepage=true]
POST /api/zones/server_room/override HTTP/1.1
Content-Type: application/json

{
  "state": "CRITICAL",
  "reason": "Scheduled drill exercise"
}
\end{lstlisting}
...
{
  "success": true,
  "message": "Zone state override applied successfully",
  "data": {
    "zone_id": "server_room",
    "state": "CRITICAL",
    "risk_score": 100.0,
...
\needspace{14\baselineskip}
\subsection{6. Priority Dispatch Queue (\texttt{GET /api/priority})}
\paragraph{Auth}: Active Session Cookie (Staff/Admin)
...
\needspace{14\baselineskip}
\subsection{7. Get Historical Incidents (\texttt{GET /api/incidents})}
\paragraph{Auth}: Active Session Cookie (Staff/Admin)
  ...
  "data": [
    {
      ...
      "zone_id": "server_room",
      "status": "OPEN",
      "hazard_types": ["fire", "gas"],
      "peak_risk_score": 73.2,
      "source": "sensor",
      "opened_at": "2026-07-26T14:30:00.000Z"
    }
  ]
}
\end{lstlisting}
...
\needspace{14\baselineskip}
\subsection{8. Get Incident Details \& Timeline (\texttt{GET /api/incidents/:id})}
\paragraph{Auth}: Active Session Cookie (Staff/Admin)
    ...
    "transitions": [
      {
        ...
        "to_state": "CRITICAL",
        "risk_score": 73.2,
        "occurred_at": "2026-07-26T14:30:00.000Z"
      }
    ]
  }
}
...
\needspace{14\baselineskip}
\subsection{9. Atomic Incident Acknowledgment (\texttt{POST /api/incidents/:id/ack})}
\paragraph{Auth}: Active Session Cookie (Staff/Admin)
...
\needspace{14\baselineskip}
\subsection{10. Short-Term Risk Trend (\texttt{GET /api/bonus/trend/:zone\_id})}
\paragraph{Auth}: Active Session Cookie (Staff/Admin)
...
\needspace{14\baselineskip}
\subsection{11. Logistic Regression ML Risk Predictor (\texttt{GET /api/bonus/ml-predict/:zone\_id})}
\paragraph{Auth}: Active Session Cookie (Staff/Admin)
...
\subsection{12. Natural Language Incident Reporting (\texttt{POST /api/bonus/nl-report})}
\paragraph{Auth}: Active Session Cookie (Staff/Admin)
...
\needspace{14\baselineskip}
\subsection{13. System Health Verification (\texttt{GET /health})}
\paragraph{Response (200 OK)}:
\begin{lstlisting}[language=json, samepage=true]
{
  "status": "ok",
  "timestamp": "2026-07-26T14:35:00.000Z",
  "database": "connected",
...
\needspace{14\baselineskip}
\subsection{14. System Stress \& Load Test Status (\texttt{GET /load-test})}
\paragraph{Response (200 OK)}:
\begin{lstlisting}[language=json, samepage=true]
{
  "status": "idle",
  "active_simulated_nodes": 0,
  "telemetry_rate_hz": 10,
  "last_run_stats": {
    "duration_seconds": 60,
    "total_payloads_ingested": 600,
    "avg_latency_ms": 12.4
  }
}
\end{lstlisting}
